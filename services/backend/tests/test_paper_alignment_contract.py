...
def test_paper_benchmark_manifest_matches_submitted_paper() -> None:
    ...
    assert data["provenance"] == "reported_in_submitted_paper_not_regenerated_by_repository_ablation_harness"
...
def test_displayed_probe_formula_matches_paper_equation_11() -> None:
    for path in DISPLAY_FILES:
        ...
        for stale in STALE_PROBE_FORMULAS:
            assert stale not in text, f"stale probe formula remains in {path.relative_to(REPO_ROOT)}"

    backend_impl = (REPO_ROOT / "services" / "backend" / "src" / "cci" / "probes" / "priority.py").read_text(encoding="utf-8")
...
def test_repository_distinguishes_paper_benchmark_from_supplementary_ablation() -> None:
    ...
    runner = (REPO_ROOT / "research" / "run_paper_experiments.py").read_text(encoding="utf-8")
    simulation = (REPO_ROOT / "services" / "backend" / "src" / "cci" / "research" / "simulation.py").read_text(encoding="utf-8")
    ...
    assert "supplementary implementation ablation" in simulation.lower()
