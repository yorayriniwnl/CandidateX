...
def test_build_candidate_manifest_from_pdf():
    ...
    assert manifest.display_name == "Jordan Example"
    assert manifest.email == "jordan@example.test"

    # Verify classified URLs
    assert len(manifest.github_urls) >= 2
    assert "https://github.com/jordan-example" in manifest.github_urls
    assert "https://github.com/jordan-example/distributed-cache" in manifest.github_urls

    assert len(manifest.linkedin_urls) == 1
    assert "https://www.linkedin.com/in/jordan-example" in manifest.linkedin_urls[0]

    assert len(manifest.deployment_urls) == 1
    assert "https://jordan-example.vercel.app" in manifest.deployment_urls[0]
