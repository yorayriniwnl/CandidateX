...
from cci.intake.technology import contains_technology, technology_pattern
...
def _term_frequency(text: str, term: str) -> int:
    """Count non-overlapping exact occurrences, including punctuation-heavy terms."""
    return sum(1 for _ in technology_pattern(term).finditer(text))
...
def extract_requirements_from_jd(
    ...
    for term in CONTROLLED_SYNONYM_MAP:
        cnt = _term_frequency(jd_lower, term)
        if cnt > 0:
    ...
    for line in lines:
        ...
        if MANDATORY_MARKERS.search(line_lower) and is_heading and len(line.split()) <= 6 and not any(contains_technology(line_lower, term) for term in CONTROLLED_SYNONYM_MAP):
            current_priority = RequirementPriority.MANDATORY
            continue
        elif PREFERRED_MARKERS.search(line_lower) and is_heading and len(line.split()) <= 6 and not any(contains_technology(line_lower, term) for term in CONTROLLED_SYNONYM_MAP):
        ...
        for term, (cap, spec) in CONTROLLED_SYNONYM_MAP.items():
            if contains_technology(line_lower, term):
