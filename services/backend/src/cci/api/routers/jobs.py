...
class JobParseRequest(BaseModel):
    ...
    role: CanonicalRole = Field(default=CanonicalRole.BACKEND, description="Target canonical role")
...
def list_jobs(organization_id: Optional[UUID] = None) -> List[JobSummaryResponse]:
        ...
        with SessionLocal() as db:
            ...
            return [
                JobSummaryResponse(
                    ...
                    created_at=j.created_at.isoformat() if hasattr(j, "created_at") and j.created_at else "",
                )
                for j in jds
            ]
