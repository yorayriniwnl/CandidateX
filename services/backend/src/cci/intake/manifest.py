...
from cci.intake.technology import normalize_technology

EMAIL_REGEX = re.compile(r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b")
NAME_NOISE_TERMS = {
    "resume", "curriculum", "vitae", "cv", "summary", "profile", "objective",
    "engineer", "developer", "programmer", "architect", "designer", "manager",
    "consultant", "scientist", "analyst", "student", "professional",
    "senior", "junior", "lead", "principal", "staff", "associate", "chief", "head",
}
ROLE_HEADER_PATTERN = re.compile(
    r"\b(?:software|data|backend|back-end|frontend|front-end|full[- ]stack|devops|machine\s+learning|technical|engineering|product|project)\s+"
    r"(?:engineer|developer|programmer|architect|designer|manager|scientist|analyst|lead)\b",
    re.IGNORECASE,
)
...
def extract_candidate_name(text: str) -> str:
    ...
    for position, line in enumerate(lines[:15]):
        ...
        if first_token in NAME_NOISE_TERMS or ROLE_HEADER_PATTERN.search(lowered):
            continue
        if len(clean_name) > 1 and len(clean_name.split()) <= 5 and re.fullmatch(
            r"[\w.'-]+(?:\s+[\w.'-]+){0,4}", clean_name, re.UNICODE
        ):
            return clean_name
    ...
    for line in skill_lines:
        ...
        for part in parts:
            ...
            if 1 <= len(clean) <= 120 and not re.match(r"^\d+$", clean):
                comparison_key = normalize_technology(clean)
                if comparison_key and comparison_key not in {normalize_technology(skill) for skill in skills}:
                    skills.append(clean)
    return skills
