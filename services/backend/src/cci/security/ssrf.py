...
def resolve_and_validate_hostname(
    ...
    if host_lower in PROHIBITED_HOSTNAMES or host_lower.endswith(
        ...
        try:
            ...
            if is_ip_restricted(ip_obj):
                raise SSRFSecurityError(
                    f"DNS rebinding / SSRF attempt: '{hostname}' resolved to restricted IP {ip_obj}"
                )
    ...
    if not resolved_ips:
        raise SSRFSecurityError(
            f"Could not resolve any valid IP addresses for '{hostname}'"
        )
    ...
    try:
        for hop in range(MAX_REDIRECTS + 1):
            ...
            with client.stream(
                "GET",
                current_url,
                headers={"User-Agent": "CandidateX-Deployment-Inspector/1.0"},
            ) as response:
                ...
                for chunk in response.iter_bytes():
                    ...
                    if total_bytes > max_bytes:
                        raise SSRFSecurityError(
                            f"Response size exceeded safety cap of {max_bytes} bytes"
                        )
