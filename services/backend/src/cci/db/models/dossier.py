...
class DossierItem(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    dossier_snapshot_id: Mapped[uuid.UUID] = mapped_column(
        GUID(),
        ForeignKey("dossier_snapshots.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    section_type: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # strong_area, uncertain_area, ownership, probe
    title: Mapped[str] = mapped_column(String(255), nullable=False)
    body_markdown: Mapped[str] = mapped_column(Text, nullable=False)
    referenced_evidence_ids: Mapped[list[Any]] = mapped_column(
        JSONType, default=list, nullable=False
    )
    item_order: Mapped[int] = mapped_column(Integer, default=0, nullable=False)

    dossier: Mapped["DossierSnapshot"] = relationship(
        "DossierSnapshot", back_populates="items"
    )
