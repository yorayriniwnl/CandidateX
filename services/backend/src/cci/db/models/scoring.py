...
class CapabilityEstimateEntity(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    estimate: Mapped[float | None] = mapped_column(
        Float, nullable=True
    )  # q_k in [0, 100], or None for UNKNOWN
    is_observed: Mapped[bool] = mapped_column(Boolean, nullable=False)
    effective_evidence_count: Mapped[float] = mapped_column(
...
class CapabilityUncertaintyEntity(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    is_low_coverage: Mapped[bool] = mapped_column(
...
class CapabilityConflictEntity(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    contradiction_diagnostic: Mapped[float] = mapped_column(
        Float, nullable=False
    )  # D_k in [-1, 1]
    has_meaningful_conflict: Mapped[bool] = mapped_column(
        Boolean, default=False, nullable=False
    )
    triggering_evidence_ids: Mapped[list[Any]] = mapped_column(
...
class AnalysisScoreEntity(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    rci: Mapped[float | None] = mapped_column(
        Float, nullable=True
    )  # Overall RCI in [0, 100], or None
