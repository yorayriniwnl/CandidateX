...
class InterviewProbePriority(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    contradiction_term: Mapped[float] = mapped_column(Float, nullable=False) # D_k based term

    questions: Mapped[List["InterviewQuestionEntity"]] = relationship(
        "InterviewQuestionEntity", back_populates="probe_priority", cascade="all, delete-orphan"
    )
...
class InterviewQuestionEntity(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    probe_priority_id: Mapped[uuid.UUID] = mapped_column(
        GUID(), ForeignKey("interview_probe_priorities.id", ondelete="CASCADE"), nullable=False, index=True
    )
    target_capability: Mapped[str] = mapped_column(String(50), nullable=False, index=True)
    question_text: Mapped[str] = mapped_column(Text, nullable=False)
    rationale: Mapped[str] = mapped_column(Text, nullable=False)
    verification_guidance: Mapped[str] = mapped_column(Text, nullable=False)
    grounding_evidence_ids: Mapped[list] = mapped_column(JSONType, default=list, nullable=False)
    suggested_followups: Mapped[list] = mapped_column(JSONType, default=list, nullable=False)
