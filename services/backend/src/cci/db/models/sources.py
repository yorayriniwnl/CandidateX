...
class RepositoryContributor(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    is_candidate_account: Mapped[bool] = mapped_column(
        Boolean, default=False, nullable=False
    )

    repository: Mapped["Repository"] = relationship(
        "Repository", back_populates="contributors"
    )
...
class Artifact(Base, UUIDPrimaryKeyMixin, ImmutableModelMixin, TimestampMixin):
    ...
    artifact_category: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # source, manifest, test, ci, db, infra
...
class RepositoryArtifact(
    ...
    repository: Mapped["Repository"] = relationship(
        "Repository", back_populates="repository_artifacts"
    )
