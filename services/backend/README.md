...
Evidence confidence uses a direct attribution gate: `confidence = path_attribution × geometric_mean(integrity, recency, verification, depth, reliability)`. This choice keeps the five evidence-quality factors on their existing [0, 1] scale while ensuring the result cannot exceed the path contribution ratio. A hard cutoff was not selected because no validated threshold exists. The output is a relative evidence weight, not a calibrated probability.
