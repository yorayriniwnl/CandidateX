...
3. `/api/live/analyze` forwards the reviewed manifest, selected role/JD, and explicit GitHub URLs. Profiles expand only through the supplied account's public repository listing. Explicit repository links take priority.
...
6. The response includes source receipts, ownership assumptions, dossier, complete graph, and scoring configuration. No public candidate lookup endpoint or cross-user dossier store is used by the deployed live service. Download JSON to retain the snapshot.
...
- Acquisition: up to 3 repositories, 60 selected text files/repository, 128 KB/file, 8 MB compressed and 24 MB expanded archive, 45-second network budget. Generated/vendor/build directories and symlinks are omitted. Receipts report omissions; a bounded scan is not a whole-repository audit.
...
- LinkedIn, coding profiles, credentials, arbitrary portfolio pages, and deployed websites are extracted and displayed but currently marked unsupported for automated live verification. No login bypass or broad identity search occurs.
