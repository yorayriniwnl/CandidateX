...
import { getAnalysisConfidence, getSourceHealth } from '../../lib/live-analysis';
      ...
      <div className={styles.tableViewport}>
        <table className={styles.dataTable}>
          ...
          <tbody>{estimates.map(item => {
            ...
            const hasInterval = item.ci_lower != null && item.ci_upper != null;
            const interval = item.ci_lower != null && item.ci_upper != null
              ? `${item.ci_lower.toFixed(1)}–${item.ci_upper.toFixed(1)}` : 'Not returned';
            const limited = !isUnknown && (globalUncertainty || item.coverage_k < 0.35 || !hasInterval);
            return (
              <tr key={item.capability_key} className={selected === item.capability_key ? styles.selectedRow : ''}>
                ...
                <td className={styles.estimateCell}>{isUnknown ? <EvidenceStatus status="unknown" label="UNKNOWN" /> : <>{item.estimate?.toFixed(1)}{limited && <EvidenceStatus status="partial" label="Limited evidence" />}</>}</td>
