...
export default function LiveAnalysisPage() {
  ...
  return <main className={styles.page}>
    ...
    <header className={styles.hero}><div className={styles.eyebrow}>Resume → Public evidence → Technical interview</div><h1>A resume is the beginning.<br />Follow the evidence.</h1><p>Explore the full résumé: skills, projects, experience, education, certificates, GitHub profiles and repositories, and public links. Review what the sources show and what still needs verification.</p></header>
    <div className={styles.layout}>
      <aside className={`${styles.panel} ${styles.controls}`}>
        ...
        {intake && <>
          ...
          <p className={styles.muted}>Up to 20 profile or repository URLs. Profiles include an inventory of up to 200 public repositories; up to 6 repositories receive a detailed file inspection. Explicit repository links take priority.</p>
          ...
          <label htmlFor="external-links">Public portfolio, certificate and other links</label><textarea id="external-links" disabled={isBusy} value={externalLinks} onChange={e => { setExternalLinks(e.target.value); setResult(null); }} placeholder="https://issuer.example/verify/credential" />
          <p className={styles.muted}>Review or add public URLs, one per line. Up to 24 pages are fetched per run. Login gates, unavailable pages and unscanned links remain visible. Certificate page matches require issuer confirmation.</p>
      ...
      <div className={styles.stack}>
        ...
        {!intake && !isBusy && <section className={`${styles.panel} ${styles.empty}`}><div className={styles.eyebrow}>Your candidate. Their actual work.</div><h2>Evidence starts with an upload.</h2><p>Inspect the résumé’s claims alongside retrieved source files, GitHub profile details, public project pages and credential links.</p><div className={styles.steps}><span>1. Upload</span><span>2. Review links</span><span>3. Fetch live</span><span>4. Inspect & export</span></div><p className={styles.muted}>Public information can support a review. Private sources, login-protected pages, issuer authentication and employment verification remain outside automated verification.</p></section>}
