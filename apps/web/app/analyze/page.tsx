...
export default function LiveAnalysisPage() {
  ...
  return <main className={styles.page}>
    ...
    <header className={styles.hero}>
      <div className={styles.eyebrow}>Resume → public evidence → technical interview</div>
      <div className={styles.heroLayout}>
        <div><h1>Understand what the resume can prove.</h1><p>Upload a resume once. We’ll organize its claims, review public technical evidence, and show what still needs a human conversation.</p></div>
        <div className={styles.heroCallout}><span className={styles.eyebrow}>Built for careful decisions</span><p>Claims, evidence, and unknowns stay separate. The result supports an interview; it never makes the hiring decision.</p></div>
      </div>
    </header>
    <div className={styles.layout}>
      <aside className={`${styles.panel} ${styles.controls}`}>
        <div className={styles.workflowHeader}><div><div className={styles.eyebrow}>Candidate review</div><h2>Start with the resume</h2></div><span className={styles.stepCount}>Step {currentStep} of 3</span></div>
        ...
        <label htmlFor="resume-upload" className={`${live.upload} ${styles.uploadCard}`}><span>Choose a resume</span><strong>PDF or DOCX</strong><small>Digital files only · up to 3 MB</small><input id="resume-upload" aria-label="Upload resume" type="file" accept=".pdf,.docx" disabled={isBusy} onChange={e => void upload(e.currentTarget.files?.[0])} /></label>
        <p className={styles.muted}>Your document is processed for this request and is not saved.</p>
      ...
      <div className={styles.stack}>
        ...
        {!intake && !isBusy && <section className={`${styles.panel} ${styles.empty}`}><div className={styles.emptyIcon} aria-hidden="true">1</div><div className={styles.eyebrow}>Ready when you are</div><h2>Upload a resume to begin</h2><p>We’ll extract the candidate’s claims first. You’ll review the public sources before anything is fetched.</p><div className={styles.steps}><span><b>1</b> Upload</span><span><b>2</b> Review</span><span><b>3</b> Analyze</span></div><p className={styles.muted}>Public evidence can support a review. Private sources, login-protected pages, issuer authentication, and employment verification remain outside automated verification.</p></section>}
        ...
        {result && <>
          ...
          <section className={`${styles.panel} ${styles.capabilityPanel}`}><div className={styles.eyebrow}>05 / Capability snapshot</div><h2>What the evidence shows</h2><p className={styles.muted}>Use the next step in each row to move from a score to a useful interview conversation.</p><CapabilitySnapshotTable estimates={result.dossier.capability_estimates} selectedCapability={capability} onSelectCapability={setCapability} /></section>
